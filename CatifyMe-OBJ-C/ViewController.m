...
@property (weak, nonatomic) IBOutlet UINavigationItem *TitleTest;
...
//Hey Austin this is where the data is made and sent to coredata but it likes to convert it to nil at the last second

//Is linked to the Like button sends liked url to coredata for storage
- (IBAction)touchLiked:(UIButton *)sender {
    NSLog(@"the url is %@", _urlDisplay.text);
    AppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
    [CatSavedData addSavedCatatTime:[NSDate date] UrlName:_urlDisplay.text inManagedObjectContext:appDelegate.managedObjectContext];
...
//Attached to the pass button does the same action as gesture tap, by loading a new cat image

- (IBAction)touchPass:(UIButton *)sender {
    [self setViewForFetching];
    SwiftConnectionLayer *swiftConnectionLayer = [SwiftConnectionLayer new];
    [swiftConnectionLayer getNewCat];
    swiftConnectionLayer.delegate = self;
